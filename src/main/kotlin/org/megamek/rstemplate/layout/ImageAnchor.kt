...
enum class ImageAnchor(val xOffset: (Double, Double) -> Double, val yOffset: (Double, Double) -> Double) {
    TOP_LEFT ({_, _ -> 0.0}, {_, _ -> 0.0}),
    TOP ({areaW, imageW -> (areaW - imageW) * 0.5}, {_, _ -> 0.0}),
    TOP_RIGHT ({areaW, imageW -> areaW - imageW}, {_, _ -> 0.0}),
    LEFT ({_, _ -> 0.0}, {areaH, imageH -> (areaH - imageH) * 0.5}),
    ...
    BOTTOM_LEFT ({_, _ -> 0.0}, {areaH, imageH -> areaH - imageH}),
